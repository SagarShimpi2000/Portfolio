...
function setupScene(gltf) {
    ...
    const container = document.getElementById('avatar-container2');
    ...
    // Create pedestal
    // Create a plane geometry for the yoga mat
const yogaMatGeometry = new THREE.PlaneGeometry(2, 1); // Adjust the width and height as needed

// Create a material for the yoga mat
const yogaMatMaterial = new THREE.MeshStandardMaterial({ color: 0xB25E75 }); // Choose a color for the mat

// Create a mesh for the yoga mat
const yogaMatMesh = new THREE.Mesh(yogaMatGeometry, yogaMatMaterial);

// Set properties for the yoga mat
yogaMatMesh.castShadow = false;
yogaMatMesh.receiveShadow = true;

// Rotate the plane to lay flat on the ground
yogaMatMesh.rotation.x = -Math.PI / 2;

// Position the yoga mat
yogaMatMesh.position.y = 0.05;

// Add the yoga mat to the scene
scene.add(yogaMatMesh);


    // Load animations
    const mixer = new THREE.AnimationMixer(avatar);
    const clips = gltf.animations;
    const waveClip = THREE.AnimationClip.findByName(clips, 'sitting');
    const stumbleClip = THREE.AnimationClip.findByName(clips, 'jumping');
